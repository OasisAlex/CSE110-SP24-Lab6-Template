describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let index = 1; index < prodItemsData.length; index++) {
      currentValue = prodItemsData[index];
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const changeCartstatus = await page.$('product-item');
    const shadowRoot = await changeCartstatus.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let allAreSame = true;
    await page.reload();
    const prodItemsStatus = await page.$$eval('product-item', prodItems => {
      return prodItems.map(item => {
        const shadowRoot = item.shadowRoot;
        const button = shadowRoot.querySelector('button');
    ...
    // Ensure all buttons say "Remove from Cart"
    for (const status of prodItemsStatus) {
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    // Check if the cart in localStorage matches the expected value
